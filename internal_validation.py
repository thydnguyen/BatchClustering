...
class internalIndex:
    ...
    def CVNN(self, data, label ):
        ...
        for index_i in self.class_iter:
            ...
            n= len(eoc)
            if n !=1:
                temp_comp = temp_comp + (sum(sy.spatial.distance.pdist(eoc,'euclidean')) * 2 / (n*(n-1)))
            for e in eoc:
                q_index_e = self.nn_exclude(data, label, e, index_i, k )

                temp_sep = temp_sep +(q_index_e / k)/(n + 1 )
            SEP.append(temp_sep)
            COM = COM + temp_comp
    ...
    def SD_valid_n(self, scat, dis, j):
